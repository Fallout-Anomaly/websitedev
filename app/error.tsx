...
export default function GlobalError(props: {
  ...
  return (
    <main className="min-h-screen bg-black text-[#e3e3e3] font-sans px-6 py-16">
      <div className="mx-auto max-w-xl">
        <h1 className="text-2xl font-black tracking-tight text-white">
          Something went wrong
        </h1>
        <p className="mt-3 text-sm text-gray-400">
          If this happened right after deploy, the most common cause is missing
          environment variables (Supabase URL/keys) in Vercel.
        </p>
        {error?.digest ? (
          <p className="mt-4 text-xs text-gray-500">
            Digest: <code className="rounded bg-white/10 px-1">{error.digest}</code>
          </p>
        ) : null}
